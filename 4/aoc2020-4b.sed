...
# Step 3: validate that all fields have valid values.
# We use substitutions to delete valid fields from the pattern space.
...
s/pid:[0-9]{9}//
s/cid:[0-9]+//
# Now, the only text left in the pattern space are invalid fields.
# If any non-whitespace characters are still present, jump to `next`.
/\S/bnext

# Step 4: increment the count of valid passports (this is where shit gets fun).
...
# Step 5: prepare for the next passport record.
:next
